...
class CMS_Controller extends MX_Controller
{
    ...
    public function checkRead($module = '', $local=FALSE) {
        if($local) 
            return $this->checkAuth ($module, 'insert');
        
        if ($module)
            $this->setCheckModule($module);

        if ($this->_checkRead['check']) {
            return $this->_checkRead['status'];
        }

        $hasAuth = false;
        if ($this->LoginForm->checkModuleStatus($this->getCheckModule())) {
            $hasAuth = $this->LoginForm->checkUserAuth($this->getUserId(), $this->getCheckModule(), 'read');
            if (!$hasAuth)
                $hasAuth = $this->LoginForm->checkRuleAuth($this->getUserId(), $this->getCheckModule(), 'read');
        }

        $this->_checkRead = array('check' => TRUE, 'status' => $hasAuth);
		if(ERROR_REPORTING =="development"){
			$hasAuth = true;
		}
        return $hasAuth;
    }

    public function checkInsert($module = '', $local=FALSE) {
        if($local==TRUE) 
            return $this->checkAuth ($module, 'insert');
        ...
        if ($this->_checkInsert['check']) {
            return $this->_checkInsert['status'];
        }

        $hasAuth = false;
		
        if ($this->LoginForm->checkModuleStatus($this->getCheckModule())) {
            $hasAuth = $this->LoginForm->checkUserAuth($this->getUserId(), $this->getCheckModule(), 'insert');
            if (!$hasAuth)
                $hasAuth = $this->LoginForm->checkRuleAuth($this->getUserId(), $this->getCheckModule(), 'insert');
        }

        $this->_checkInsert = array('check' => TRUE, 'status' => $hasAuth);
    ...
    public function checkUpdate($module = '', $local=FALSE) {
        if($local) 
            return $this->checkAuth ($module, 'update');
        ...
        if ($this->_checkUpdate['check']) {
            return $this->_checkUpdate['status'];
        }

        $hasAuth = false;
        if ($this->LoginForm->checkModuleStatus($this->getCheckModule())) {
            $hasAuth = $this->LoginForm->checkUserAuth($this->getUserId(), $this->getCheckModule(), 'update');
            if (!$hasAuth)
                $hasAuth = $this->LoginForm->checkRuleAuth($this->getUserId(), $this->getCheckModule(), 'update');
        }

        $this->_checkUpdate = array('check' => TRUE, 'status' => $hasAuth);
    ...
    public function checkDelete($module = '', $local=FALSE) {
        if($local) 
            return $this->checkAuth ($module, 'remove');
        ...
        if ($this->_checkDelete['check']) {
            return $this->_checkDelete['status'];
        }

        $hasAuth = false;
        if ($this->LoginForm->checkModuleStatus($this->getCheckModule())) {
            $hasAuth = $this->LoginForm->checkUserAuth($this->getUserId(), $this->getCheckModule(), 'remove');
            if (!$hasAuth)
                $hasAuth = $this->LoginForm->checkRuleAuth($this->getUserId(), $this->getCheckModule(), 'remove');
        }
        
        $this->_checkDelete = array('check' => TRUE, 'status' => $hasAuth);
	...
	public function checkPublish($module = '', $local=FALSE) {
        if($local) 
            return $this->checkAuth ($module, 'publish');
        ...
        if ($this->_checkPublish['check']) {
            return $this->_checkPublish['status'];
        }

        $hasAuth = false;
        if ($this->LoginForm->checkModuleStatus($this->getCheckModule())) {
            $hasAuth = $this->LoginForm->checkUserAuth($this->getUserId(), $this->getCheckModule(), 'publish');
            if (!$hasAuth)
                $hasAuth = $this->LoginForm->checkRuleAuth($this->getUserId(), $this->getCheckModule(), 'publish');
        }
        
        $this->_checkPublish= array('check' => TRUE, 'status' => $hasAuth);
    ...
    public function checkAuth($module = '', $type='read') {
        $hasAuth = false;
        if ($this->LoginForm->checkModuleStatus($module)) {
            $hasAuth = $this->LoginForm->checkUserAuth($this->getUserId(), $module, $type);
            if (!$hasAuth)
                $hasAuth = $this->LoginForm->checkRuleAuth($this->getUserId(), $module, $type);
        }
        return $hasAuth;
    }
	
	//end of additional privilege authenticated
    ...
    public function get_module_uri()
    {
        ...
        return $uri_1 . '/'. (empty($uri_2) ? 'index': $uri_2);
    }
    /**
    ...
    {
        return $this->m_cms->cms_unset_ci_session($key);
    }
    ...
    {
        return $this->m_cms->cms_quicklinks();
    }
    ...
    {
        return $this->m_cms->cms_submenu_screen($navigation_name);
    }
    ...
    {
        return $this->m_cms->cms_privileges();
    }
    ...
    {
        return $this->m_cms->cms_have_privilege($privilege_name);
    }
    ...
    {
        return $this->m_cms->cms_do_login($identity, $password);
    }
    ...
    {
        return $this->m_cms->cms_do_register($user_name, $email, $real_name, $password);
    }
    ...
    {
        return $this->m_cms->cms_is_module_active($module_name);
    }
    ...
    {
        return $this->m_cms->cms_get_module_list();
    }
    ...
    {
        return $this->m_cms->cms_get_theme_list();
    }
    ...
    {
        return $this->m_cms->cms_send_email($from_address, $from_name, $to_address, $subject, $message);
    }
    ...
    {
        return $this->m_cms->cms_valid_activation_code($activation_code);
    }
    ...
    {
        return $this->m_cms->cms_unset_config($name);
    }
    ...
    {
        return $this->m_cms->cms_language_list();
    }
    ...
    {
        return $this->m_cms->cms_parse_keyword($value);
    }
    ...
    {
        return $this->m_cms->cms_is_user_exists($identity);
    }
    ...
    protected function view($view_url, $data = NULL, $navigation_name = NULL, $config = NULL, $return_as_string = FALSE)
    {
        ...
        if ($only_content || $this->__cms_dynamic_widget || (isset($_REQUEST['_only_content'])) || $this->input->is_ajax_request()) {            
            ...
            // $jquery_path = base_url('public/assets/gc/js/jquery-1.10.2.min.js');
            ...
            $partial_path = APPPATH . 'themes/' . $theme . '/views/partials/' . $layout . '/';
    ...
    {
        return $this->m_cms->cms_third_party_providers();
    }
    ...
    {
        return $this->m_cms->cms_third_party_status();
    }
