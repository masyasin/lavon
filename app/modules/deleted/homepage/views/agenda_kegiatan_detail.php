...
<div class="container animsition">
  <ol itemscope="" class="breadcrumbs" itemtype="https://schema.org/BreadcrumbList">
    ...
    <li itemprop="itemListElement" itemscope="" itemtype="http://schema.org/ListItem">
      ...
      <a itemprop="item" href="{{ base_url }}informasi/agenda-kegiatan">
  ...
  <div class="col-md-9 col-no-gutter">
    <article class="headline-news-single withsubtitle">
      <header>
        <h1 class="title"><?=$akg['judul']?></h1>
      </header>
      <figure class="hero">
       
        <img class="img-responsive lazy" style="width:100%" data-src="<?=$akg['gambar']?>"  
             src="data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw=="
             alt="<?=$akg['keterangan_gambar']?>" 
             title="<?=$akg['keterangan_gambar']?>">
        
        <figcaption>
          
          <?=$akg['keterangan_gambar']?>
      ...
      <div class="content-text-editor">
        <div class="example-wrap">
                <h4 class="example-title"><?=$akg['judul']?></h4>
                <p><?=$akg['keterangan']?>.</p>
                <dl class="dl-horizontal">
                  <dt>Hari/Tanggal</dt>
                  <dd><?=$akg['tanggal_txt']?></dd>
                  <dt>Waktu</dt>
                  <dd> Pukul <?=$akg['waktu']?> WIB.</dd>
        ...
        <?= !empty($akg['file']) ? '<div class="attachments">Lampiran : <a class="" target="_blank" href="'.site_url('public/assets/uploads/files/attachments').'/'.$akg['file'].'"><i class="icon wb-download"></i> '.$akg['file'].'</a></div>' : '' ?>
      </div>
      <?if(!empty($related)):?>
  <section class="related-news berita-terkait">
    ...
    <ul class="row">
      ...
      <li class="col-md-3">
        <a href="{{ base_url }}informasi/agenda-kegiatan/<?=$r->slug?>" class="thumbnail">
