      ...
      <div class="content-text-editor">
       ...
       <section class="latest-topics listing  col-no-gutter"">
          <ul class="media-list main-list">
             <?foreach($id as $i ):?>
              <?
              $imagurl = my_simple_crypt('public/assets/uploads/files/info_dinas/' . $i['gambar'],'e');
              $imagurl = base_url(). 'files/thumb/'.$imagurl.'/300/225';
              ?>
              <li class="media" v-for="article in articles">
              <a class="thumb pull-left" href="{{ base_url }}/informasi/info-dinas/<?=$i['slug']?>">
              <img alt="<?=$i['keterangan_gambar']?>" class="media-object lazy" data-src="<?=$imagurl?>" src="data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw==">
              </a>
              <div class="media-body">
              <h4 class="media-heading"><a href="{{ base_url }}/informasi/info-dinas/<?=$i['slug']?>"><?=$i['judul']?></a></h4>
              <span class="">
                <p><?=html2text(substr($i['isi'],0, 290))?>...</p>
              </span>
